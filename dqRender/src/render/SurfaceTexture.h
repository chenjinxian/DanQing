...
//
// §3.4 deviation: itwinjs uses VertexLUT (g_vertLutData + unquantize2d);
// DanQing uses a_texCoord attribute. TODO Step 3 VertexLUT.
...
//
// §3.4 deviation: computeTexCoord uses a_texCoord attribute (vec2) instead of
// itwinjs's VertexLUT path (g_vertLutData3/4 + decodeUInt16 + unquantize2d).
// TODO Step 3 VertexLUT.
// Simplified: sampleSurfaceTexture omits constantLod branch; computeBaseColor
// omits glyph/whiteOnWhite. TODO follow-up.
// ---------------------------------------------------------------------------
inline void addTexture(ProgramBuilder& builder)
{
    ...
    // u_qTexCoordParams uniform — quantized texture coordinate unquantization.
    // Ported from: itwinjs-core Surface.ts addTexture() (line 583-594)
    frag.addUniform("u_qTexCoordParams", VariableType::Vec4, nullptr);
