...
QPixmap FileCardDelegate::loadAndCacheThumbnail(const QString& path, int thumbnailSize) const
{
    ...
    if (path.endsWith(QLatin1String(".fcstd"), Qt::CaseSensitivity::CaseInsensitive)) {
        ...
        // existed. Use document-new.svg as placeholder (freecad-doc.svg not bundled).
        QImageReader reader(QLatin1String(":/icons/document-new.svg"));
        ...
        // MacroEditor.svg not bundled; use accessories-text-editor.svg as placeholder.
        QImageReader reader(QLatin1String(":/icons/accessories-text-editor.svg"));
        reader.setScaledSize(QSize(thumbnailSize, thumbnailSize));
        thumbnail = QPixmap::fromImage(reader.read());
    }
