...
// Ported from: imodel-native iModelCore/Bentley/PublicAPI/Bentley/btree/btree.h
//   Original upstream: Google cpp-btree (https://github.com/abedra/cpp-btree), Kronuz fork
//   (C) 2019 German Mendez Bravo, (C) 2013 Google Inc. Apache License 2.0.
//   Identifier rename: btree_* -> DQ_BASE_BTREE_* / namespace adjustments (mechanical);
//   BeAssert -> DqAssert; include paths repointed to dqBase/bpair.h, dqBase/DqAllocator.h.
//   CLAUDE.md §7.3: integrated as source (not external dep). Do not replace with std::map/std::set —
//   cache-friendly B-tree, 3-10x faster than std::map on dqBase hot paths (architectural decision).
/*---------------------------------------------------------------------------------------------
...
// btree 来自 Google cpp-btree / imodel-native 移植（vendored）。
// 其容器/迭代器沿用 user-provided copy ctor + 隐式 copy assign 的旧式写法，
// 在 C++20 -Wextra -Werror 下触发 -Wdeprecated-copy-with-user-provided-copy。
// 修改 btree 类定义会偏离上游源码，故在此 vendored 头文件局部抑制该告警。
...
struct btree_map_params : public btree_common_params<Key, Compare, Alloc, TargetNodeSize, sizeof(Key) + sizeof(Data)> {
	...
	static void swap(value_type& a, value_type& b) {
		dqBase::btree_swap_helper(const_cast<Key&>(a.first), const_cast<Key&>(b.first));
		dqBase::btree_swap_helper(a.second, b.second);
	}
...
void btree_node<P>::swap(btree_node* x) {
	...
	// Swap the counts.
	dqBase::btree_swap_helper(fields_.count, x->fields_.count);
}
