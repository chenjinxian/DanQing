...
// Ported from: imodel-native iModelCore/Bentley/PublicAPI/Bentley/bpool.h
//   Original upstream: Boost.Pool boost/pool/pool.hpp (BSL 1.0, (C) 2000-2001 Stephen Cleary).
//   Identifier rename: BBOOST_* -> DQ_BASE_*, BeAssert -> DqAssert (mechanical).
//   CLAUDE.md §7.3: integrated as source (not external dep). Do not replace with std::malloc —
//   this is a fixed-size block allocator on a performance-critical path (architectural decision).
/*---------------------------------------------------------------------------------------------
...
#ifndef DQ_BASE_POOL_HPP
#define DQ_BASE_POOL_HPP
#include "BeAssert.h"
#include <type_traits>
    ...
    #undef min
#endif
#define DQ_BASE_NO_INCLASS_MEMBER_INITIALIZATION
#define DQ_BASE_ASSERT(X) DqAssert((X))

#  ifdef DQ_BASE_NO_INCLASS_MEMBER_INITIALIZATION
#       define DQ_BASE_STATIC_CONSTANT(type, assignment) enum { assignment }
#  else
#     define DQ_BASE_STATIC_CONSTANT(type, assignment) static const type assignment
#  endif
...
//#define DQ_BASE_PTR_SIZE math::static_lcm<sizeof(size_type), sizeof(void *)>::value
#define DQ_BASE_PTR_SIZE sizeof(void *)
//#define DQ_BASE_MIN_ALIGNMENT math::static_lcm< std::alignment_of<void *>::value, std::alignment_of<size_type>::value>::value
#define DQ_BASE_MIN_ALIGNMENT std::alignment_of<void *>::value

// boost::simple_segregated_storage
#include "simple_segregated_storage.h"

#ifdef DQ_BASE_POOL_INSTRUMENT
#include <iostream>
#include<iomanip>
#endif
#ifdef DQ_BASE_POOL_VALGRIND
#include <set>
...
*/
  ...
  private:
    ...
    {
      return (ptr_next_size() - DQ_BASE_PTR_SIZE);
    }
  ...
  public:
    ...
    { //! \returns size of element pointer area.
      return static_cast<size_type>(sz - sizeof(size_type) - DQ_BASE_PTR_SIZE);
    }
...
#ifndef DQ_BASE_POOL_VALGRIND
/*!
  \brief A fast memory allocator that guarantees proper alignment of all allocated chunks.

  \details Whenever an object of type bpool needs memory from the system,
  ...
  Users may pass an additional constructor parameter to bpool.
  This parameter is of type size_type,
  and is the number of chunks to request from the system
  the first time that object needs to allocate system memory.
  The default is 32. This parameter may not be 0.

  Users may also pass an optional third parameter to bpool's
  ...
  being allocated, the bpool will backtrack just once, halving
  ...
  There are essentially two ways to use class bpool: the client can call \ref malloc() and \ref free() to allocate
  and free single chunks of memory, this is the most efficient way to use a bpool, but does not allow for
...
class bpool: protected simple_segregated_storage < typename UserAllocator::size_type >
{
  ...
  private:
    DQ_BASE_STATIC_CONSTANT(size_type, min_alloc_size = DQ_BASE_PTR_SIZE );
    DQ_BASE_STATIC_CONSTANT(size_type, min_align = DQ_BASE_MIN_ALIGNMENT );
  ...
  protected:
    ...
    { //! \param chunk chunk to check if is from this bpool.
      ...
      //!
      //! Returns false if chunk was allocated from some other bpool,
      //! or may be returned as the result of a future allocation from some other bpool.
    ...
    { //!  Calculated size of the memory chunks that will be allocated by this Pool.
      ...
      if(rem)
         s += min_align - rem;
      DQ_BASE_ASSERT(s >= min_alloc_size);
      DQ_BASE_ASSERT(s % min_align == 0);
      return s;
    }
  ...
  public:
    ...
    { //! Allocates a chunk of memory. Searches in the list of memory blocks
      ...
      //! Otherwise, creates a new memory block, adds its free list to bpool's free list,
    ...
    { //! \returns Returns true if chunk was allocated from u or
      ...
      //! Returns false if chunk was allocated from some other bpool or
      //! may be returned as the result of a future allocation from some other bpool.
      //! Otherwise, the return value is meaningless.
      //! Note that this function may not be used to reliably test random pointer values.
...
#ifndef DQ_BASE_NO_INCLASS_MEMBER_INITIALIZATION
template <typename UserAllocator>
...
{ //! bpool must be ordered. Frees every memory block that doesn't have any allocated chunks.
...
{ //! No memory in any of our storages; make a new storage,
  ...
  size_type POD_size = static_cast<size_type>(next_size * partition_size + DQ_BASE_PTR_SIZE + sizeof(size_type));
  ...
  if (ptr == 0)
  {
     if(next_size > 4)
     {
        ...
        POD_size = static_cast<size_type>(next_size * partition_size + DQ_BASE_PTR_SIZE + sizeof(size_type));
...
public:
  ...
  explicit bpool(const size_type s, const size_type = 32, const size_type m = 0) : chunk_size(s), max_alloc_size(m) {}
  ~bpool()
  {
     purge_memory();
  }
  ...
  void free (void *const chunk)
  {
     DQ_BASE_ASSERT(used_list.count(chunk) == 1);
     DQ_BASE_ASSERT(free_list.count(chunk) == 0);
  ...
  void free (void *const chunk, const size_type)
  {
     DQ_BASE_ASSERT(used_list.count(chunk) == 1);
     DQ_BASE_ASSERT(free_list.count(chunk) == 0);
     used_list.erase(chunk);
...
protected:
   ...
   // NOTE: std::set<void*> is a 1:1 port of imodel-native bpool.h (which also uses
   // std::set<void*> for free_list/used_list — see ref lines 867/877/882/971). This is
   // NOT a §8.3 violation: §0/§12.7 require 1:1 fidelity to the C++ reference, and the
   // reference here chose std::set<void*>. Registered as TD-9. Do not "fix" to bset.
